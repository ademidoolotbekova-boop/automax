  ...
  describe "GET /admin/audit_logs" do
    ...
    context "when authenticated as super admin" do
      it "returns success" do
        get admin_audit_logs_path, headers: auth_headers(owner_user)
      ...
      it "filters by search term" do
        get admin_audit_logs_path, params: { search: "Updated" }, headers: auth_headers(owner_user)
        expect(response).to have_http_status(:success)
      end

      it "filters by action" do
        get admin_audit_logs_path, params: { action_filter: "update" }, headers: auth_headers(owner_user)
        expect(response).to have_http_status(:success)
      end

      it "supports sorting" do
        get admin_audit_logs_path, params: { sort_column: "created_at", sort_direction: "asc" }, headers: auth_headers(owner_user)
        expect(response).to have_http_status(:success)
      end
    end
  end
